...
function parseCard(id, text) {
  ...
  // closing heading at any level (DONE, RESOLVED, MERGED, FIXED or CLOSED, in capitals) on a
  ...
  if (col !== 'done') {
    const closing = /^#{1,6}\s+(DONE|RESOLVED|MERGED|FIXED|CLOSED)(?=[\s:]|$).*$/m.exec(card.evidence)
    if (closing) {
