...
**AND IT WANTS SOMEONE COLD, which is the same argument that put `bug-sync138` with Kenya.** `CONTRIBUTING.md`'s *Writing it down* says the warnings ARE the product, and that the way to check a sentence is to have a peer read it cold. Rook has never read the audit or lint output. That is not a gap to work around here — it is the qualification.
