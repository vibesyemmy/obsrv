...
**Verified, and what was not.** The direct-push path is verified by this commit: it reached `main` under the new protection, which both proves admins are not gated and demonstrates the gap above rather than asserting it. **The PR gate itself is UNEXERCISED** — nobody has watched it refuse a pull request, so by this repo's own standard it is a claim rather than a check until the next PR tests it. Said here rather than discovered later.
