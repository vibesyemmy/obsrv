...
- The settings drawer's number fields keep a local draft and commit only a finite value of at least their `min`: a controlled input bound straight to the store would commit "2" on the way to "27", and `ppi` throws on it while main's `parseSettings` refuses it. A refusal from main (the IPC rejects) rolls the store back and shows an inline `.field-error` rather than a toast — `Toast` does not exist until Task 16. The §9 fallback notice keys off `selectScaleIsFallback`, which also covers a zero diagonal, not only an unknown display (Task 15).
...
npm run test:e2e
```
Expected: the thirty-nine tests from Tasks 7–14 still pass; all six new tests
...
list): the store and main only ever see a finite value of at least the field's
`min`, a half-typed value shows a `.field-error` line, and leaving the field
snaps it back. Settings commit to the store first so the panes follow the
keystroke; if main rejects the value the store is rolled back and the line says
so.
...
npm run typecheck && npm run test:e2e
```
Expected: typecheck silent; Playwright `45 passed` (39 from Tasks 7–14, 6 new).
