...
- Modify: `src/shared/types.ts` (add `ScrollPos`), `src/main/nativePane.ts` (sync preload; URL reporting moves to SyncBus), `src/main/targetSource.ts` (sync preload), `src/main/context.ts` (add `sync`), `src/main/ipc.ts` (announce deliberate double loads), `src/main/index.ts` (attach it)
- Test: `tests/e2e/sync.spec.ts`
...
npm run test:e2e
```
Expected: the twenty tests from Tasks 7–10 still pass; all six new tests fail —
the scroll ones because `target` stays at 0, the navigation ones because the
other pane keeps its old URL.
 ...
 * target — so their scroll offsets track each other.
 *
...
function report(): void {
  ...
  ipcRenderer.send(IPC.syncScroll, { x: window.scrollX, y: window.scrollY })
}
...
ipcRenderer.on(IPC.applyScroll, (_e, pos: ScrollPos) => {
...
export function attachSyncBus(
  ...
  const onScroll = (e: IpcMainEvent, pos: ScrollPos): void => {
    // Route by sender identity; the app's own renderer never uses this channel.
    const other =
...
/** Everything the IPC layer and the test hooks need. */
export interface AppContext {
  win: BrowserWindow
  native: NativePane
  target: TargetSource
...
and replace the `IPC.navigate` handler with:
```ts
  ipcMain.handle(IPC.navigate, async (_e, url: string) => {
    ...
    // it does not mirror the pair back and trigger a second load.
    sync.expect(wanted)
...
function boot(): void {
  ...
  const target = new TargetSource()
  target.on('load-error', err => win.webContents.send(IPC.loadError, err))
  target.on('loading', loading => win.webContents.send(IPC.targetLoading, loading))

  const bus = attachFrameBus(target, win)
  // SyncBus owns URL reporting for both panes, so the URL bar sees exactly one
  // update per navigation whichever pane started it.
  const sync = attachSyncBus(native, target, url => win.webContents.send(IPC.urlChanged, url))
...
npm run typecheck && npm run test:e2e
```
Expected: typecheck silent; Playwright `26 passed` (20 from Tasks 7–10, 6 new).
...
```bash
git add src/main src/preload/sync.ts src/shared/types.ts tests/e2e/sync.spec.ts tests/fixtures/tall.html && git commit -m "feat(main): mirror scroll and navigation between the panes"
```
