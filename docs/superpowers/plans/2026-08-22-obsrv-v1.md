...
```
Expected: `3 passed` for the unit file; `19 passed` for the browser project
(15 from Task 12, 4 new — the fourth proves a transparent PNG region comes out
white, not black).
...
because the drawing buffer is not preserved):
```ts
import { test, expect, type ElectronApplication, type Page } from '@playwright/test'
import { resolve } from 'node:path'
import { pathToFileURL } from 'node:url'
...
npm run test:e2e
```
Expected: the forty-six tests from Tasks 7–15 still pass; all four new tests
fail — there is no `.toast`, no drop handling and no menu yet.

- [ ] **Step 8: The menu channel**

In `src/shared/ipc.ts`, add two entries to the `IPC` object:
```ts
  openImage: 'obsrv:open-image',
  focusUrl: 'obsrv:focus-url',
```

In `src/shared/api.ts`, add two methods to `ObsrvApi`:
```ts
...
queries and error-clearing `onUrlChanged`, and Task 15's `Drawer` prop):
```tsx
...
npm run typecheck && npm run test:e2e
```
Expected: typecheck silent; Playwright `50 passed` (46 from Tasks 7–15, 4 new).
Run it twice.
