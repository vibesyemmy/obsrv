...
- `IPC.openImage` is a nineteenth channel, added beyond the spec's list so File → Open Image can nudge the renderer to open its own picker. No file bytes cross IPC; drag-drop and the menu share one decode path (Task 16).
...
- Modify: `src/preload/app.ts` (replace the Task 1 stub), `src/main/context.ts` (add `bus`), `src/main/index.ts` (attach it)
...
If `frames reach the renderer` finds zero frames, the ready gate never opened —
confirm `did-finish-load` fires by checking that the window actually loaded the
renderer (a failed `loadFile` leaves the gate shut for the whole session).

- [ ] **Step 8: Commit**

```bash
git add src/main src/preload/app.ts tests/e2e/frame-bus.spec.ts && git commit -m "feat(main): ship 1x frames to the renderer over IPC"
```
 ...
 *    static page the first paint is often the only paint — the canvas would
 *    stay blank forever.
 *  - Whenever delivery (re)opens — renderer load, or leaving image mode — the
 *    target is invalidated so the canvas gets a full frame immediately instead
 *    of waiting for the page to damage itself.
 *
 * Throttling belongs to `TargetSource` (`setFrameRate`), not here.
 */
export function attachFrameBus(target: TargetSource, win: BrowserWindow): FrameBus {
...
const api: ObsrvApi = {
  ...
  onFrame: cb => subscribe<FrameMessage>(IPC.frame, cb),
