...
leave a stale entry that swallows a later genuine mirror. Two mirrors that
reverse direction within a second are treated as a loop (a SPA rewriting its
URL on load) and the second is dropped with one `console.warn`; a test that
reverses direction on purpose must wait out that window first.

**Files:**
- Create: `src/preload/sync.ts` (replace the Task 1 stub), `src/main/syncBus.ts`, `tests/fixtures/tall.html`, `tests/fixtures/redirect.html`
...
`parseScrollPos` unit cases fail on a missing export); six of the seven new
tests fail — the scroll ones because `target` never arrives, the navigation
ones because the other pane keeps its old URL. "Exactly once" passes trivially
until something mirrors; it only bites once SyncBus exists without `expect`.
...
```
Expected: typecheck silent; Vitest `91 passed`; Playwright `29 passed` (22
from Tasks 7–10, 7 new).
...
```bash
git add electron.vite.config.ts src/main src/preload/sync.ts src/shared/types.ts src/shared/ipcPayloads.ts tests/unit/ipcPayloads.test.ts tests/e2e/sync.spec.ts tests/fixtures/tall.html tests/fixtures/redirect.html && git commit -m "feat(main): mirror scroll and navigation between the panes"
```
