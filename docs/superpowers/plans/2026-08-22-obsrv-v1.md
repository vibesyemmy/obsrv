...
npm run test:e2e
```
Expected: the four Task 7 tests still pass; all six new tests fail with
...
alias typed `void` in Electron 43.
...
npm run typecheck && npm run test:e2e
```
Expected: typecheck silent; Playwright `10 passed` (4 from Task 7, 6 new).
