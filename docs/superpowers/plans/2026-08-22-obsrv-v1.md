...
npm run test:e2e
```
Expected: the thirteen tests from Tasks 7–9 still pass; all seven new tests fail
...
export function registerIpc(ctx: AppContext): void {
  ...
  ipcMain.on(IPC.forward, () => {
    native.forward()
    target.forward()
  })

  // --- target ---------------------------------------------------------------
  ipcMain.handle(IPC.setViewport, (_e, width: number, height: number) => {
    const v = target.setViewport(width, height)
    return { width: v.width, height: v.height }
  })
  ipcMain.on(IPC.sendInput, (_e, ev: TargetInputEvent) => target.sendInput(ev))

  // --- mode -----------------------------------------------------------------
  ipcMain.on(IPC.setMode, (_e, mode: 'url' | 'image') => {
    const live = mode === 'url'
  ...
  ipcMain.on(IPC.setNativeBounds, (_e, rect: Rect) => {
    rendererDrivesLayout = true
    native.setBounds(rect)
  })

  // --- host display ---------------------------------------------------------
  ipcMain.handle(IPC.getHostInfo, () => hostInfo(win))
...
function boot(): void {
  ...
  // These two forwards live here rather than in `registerIpc` because
  // `NativePane` takes its callbacks at construction time.
  const native = new NativePane(win, {
...
npm run typecheck && npm run test:e2e
```
Expected: typecheck silent; Playwright `20 passed` (4 + 6 + 3 from Tasks 7–9,
7 new).
