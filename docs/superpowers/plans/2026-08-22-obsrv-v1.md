...
export function simulatePixel(rgb: RGB, params: PanelParams, x = 0, y = 0): RGB {
  ...
  const d = params.dither ? (bayer(x, y) - 0.5) / params.levels : 0
...
- Modify: `src/main/context.ts` (add `target`), `src/main/index.ts` (construct and wire it)
...
test('rasterises at 1x: frame size equals the CSS viewport', async () => {
  const seen = await app.evaluate(async (_electron, url: string) => {
    const ctx = (globalThis as any).__obsrv
...
test('forwards clicks into the offscreen page', async () => {
  const title = await app.evaluate(async () => {
    ...
    const common = { button: 'left', clickCount: 1, modifiers: [] as string[] }
    ...
    wc.on('paint', (_event, dirty, image) => {
      ...
      this.emit('frame', {
        frame: {
          ...
          data: new Uint8Array(image.crop(dirty).getBitmap()),
        },
...
If `rasterises at 1x` reports `2732x1536` on a Retina host, the offscreen
surface is following the host scale factor — check that `offscreen` is the
object form (`{ deviceScaleFactor: 1 }`) and not the legacy `offscreen: true`.
...
```bash
git add src/main tests/e2e/target-source.spec.ts && git commit -m "feat(main): offscreen 1x target source emitting dirty-rect BGRA frames"
```
...
void main() {
  ...
  c *= uBrightness;                                 // peak brightness
  c = uBlackFloor + (1.0 - uBlackFloor) * c;        // contrast / black floor
  float l = dot(c, vec3(0.2126, 0.7152, 0.0722));
  c = vec3(l) + (c - vec3(l)) * uGamut;             // gamut coverage
  c = pow(clamp(c, 0.0, 1.0), vec3(1.0 / GAMMA));   // encode

  float d = uDither * (bayer4(t) - 0.5) / uLevels;
...
```ts
import type { TargetInputEvent } from '../../../shared/types'
...
export function modifiersOf(e: Omit<KeyLike, 'key'>): string[] {
  const m: string[] = []
