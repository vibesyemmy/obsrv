...
npm run test:e2e
```
Expected: the fourteen tests from Tasks 7–9 still pass; all seven new tests fail
with `TypeError: window.obsrv.navigate is not a function` (and
`...setMode is not a function` in the last one) — the preload still exposes only
`onFrame`.
...
`setSettings` saves before it assigns — `saveSettings` rejects non-finite
values that the `> 0` check lets through (`Infinity`), and the in-memory copy
must never hold a value disk refused.

- [ ] **Step 6: Widen the preload to the whole API**
...
- [ ] **Step 7: Hand layout and channels to registerIpc**
...
npm run typecheck && npm run test:e2e
```
Expected: typecheck silent; Playwright `21 passed` (4 + 7 + 3 from Tasks 7–9,
7 new).
