...
test('the target canvas shows the page, not a blank', async () => {
  ...
  //
  // A third instance, 2026-09-26: a `page.press` actionability timeout on
  // line 93 below, misread in-room as a canvas-blank sighting by three
  // people in turn before the trace was actually opened. See
  // `docs/e2e-flakes.md`'s "panes.spec.ts:93" entry — this comment records
  // that the misreading happens; that doc records the mechanism.
  await page.fill('.url-form input', FIXTURE)
  await page.press('.url-form input', 'Enter')
  await expect.poll(paneUrls).toEqual({ native: FIXTURE, target: FIXTURE })
